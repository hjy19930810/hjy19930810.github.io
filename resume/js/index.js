$(function(){
    ...
    'some-other-header': 'some value'
		...
		success: function(data){
			data.about_info.forEach(function(item){
				$("#about_info").append("<p class='ajax_info'>"+item+"</p>");
			});
    ...
    $('#dowebok').fullpage({
		...
		afterLoad: function(anchorLink,index){
			if(index==1){
				$(".nav li").children().removeClass("act");
				$(".page1").addClass("act");
				$("aside a").eq(0).addClass("selected").siblings().removeClass("selected");
			}
			if(index==2){
...
// 图片轮播
	...
	var page=1;
	$("#exp_timeline a.timecolor1").click(function(){
		$("a").removeClass("timecolor");
		$(".timecolor1").addClass("timecolor");
		$("#exp_list_content").stop(true,false).animate({left:-$(".exp_list").width()*$(this).index()},2000,"easeInOutCubic");
		page=$(this).index()+1;
		});
	$("#exp_timeline a.timecolor2").click(function(){
		$("a").removeClass("timecolor");
		$(".timecolor2").addClass("timecolor");
		$("#exp_list_content").stop(true,false).animate({left:-$(".exp_list").width()*$(this).index()},2000,"easeInOutCubic");
		page=$(this).index()+1;
		});
	$("#exp_timeline a.timecolor3").click(function(){
		$("a").removeClass("timecolor");
		$(".timecolor3").addClass("timecolor");
		$("#exp_list_content").stop(true,false).animate({left:-$(".exp_list").width()*$(this).index()},2000,"easeInOutCubic");
		page=$(this).index()+1;
		});
	$("#exp_list_toleft").click(function()
    {
		if(!$("#exp_list_content").is(":animated")){
			...
			page--;
			num=page-1;
			$(".color a:eq("+num+")").addClass("timecolor");
		}
	});
...
if($(window).width()<=768){
	flag=false;
	var arr = ["首页","关于我","专业技能","学习经历","我的作品","联系方式"];
	var fragment = document.createDocumentFragment();
	$.each(arr,function(i,item){
		num=i+1;
		if(num==1){var newItem = $("<li class='new'><a class='act page"+num+"' href='#page"+num+"'>"+item+"</a></li>")[0];}
		else{var newItem = $("<li class='new'><a class='page"+num+"' href='#page"+num+"'>"+item+"</a></li>")[0];}
...
var flag2=true;		
if($(window).width()<=480){
	$("#about_en_edu,#about_en_age,#about_en_loc,#about_en_sta").hide();
}
...
$(window).resize(function(){
	if($(window).width()<=480){
		...
		if($(window).width()>480&&flag2){
		$("#about_en_edu,#about_en_age,#about_en_loc,#about_en_sta").show();		
		flag2=false;		
	}
})
